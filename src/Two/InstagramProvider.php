...
class InstagramProvider extends AbstractProvider implements ProviderInterface
{
     ...
     * @var string
     */
    protected $graphUrl = 'https://api.instagram.com';
     ...
     * @return string
     */
    public function __getAccessToken($code)
    {
     ...
     * {@inheritdoc}
     */
    protected function __parseAccessToken($body)
    {
        $body = json_decode($body, false);
        //parse_str(json_decode($body, true));
     ...
     * {@inheritdoc}
     */
    protected function __getUserByToken($token)
    {
     ...
     * {@inheritdoc}
     */
    protected function __mapUserToObject(array $user)
    {
    ...
    protected function mapUserToObject(array $user)
    {
        ...
        $avatarUrl = !empty($user["profile_picture"])? $user["profile_picture"] : "";

        $firstName = isset($user['full_name']) ? $user['full_name'] : null;
        $fullName = isset($user['full_name']) ? $user['full_name'] : null;

        $lastName = isset($user['full_name']) ? $user['full_name'] : null;

        return (new User)->setRaw($user)->map([
            'id' => $user['id'], 'nickname' => null, 'name' => $fullName,
            'email' => isset($user['email']) ? $user['email'] : null, 'avatar' => $avatarUrl,
  ...
  /*public function __construct($config) {
    if (true === is_array($config)) {
      ...
      $this->setApiKey($config['client_id']);
      $this->setApiSecret($config['client_secert']);
      $this->setApiCallback($config['redirect']);
  ...
  public function getUserByToken($token) {
    ...
    $responseRaw = $this->_makeCall('users/' . $id, $auth);
    $responseRaw = json_decode(json_encode($responseRaw), true);
   ...
   * @return string
   */
  public function getAccessToken($code = false, $token = false) {

    if(false === $code) {
        return $this->_accesstoken;
    } else {

        $apiData = $this->getTokenFields($code);
